...
export default function MusicNFTStudio() {
  ...
  setTimeout(() => setToast({ show: false, message: '' }), 4000); // Tự biến mất sau 4 giây
};
...
const handleRegisterEmail = async () => {
  if (!authEmail) return showNotification("⚠️ Vui lòng nhập Email!");
  const { error } = await supabase.from('profiles').insert([{ email: authEmail }]);
  if (error) {showNotification("❌ Lỗi: " + error.message);
  }else {
    showNotification(`🎉 Chào mừng ${authEmail} gia nhập Studio!`);
    setShowAuthModal(false); // Đóng popup
  }
};
...
const connectMetamask = async () => {
  if (window.ethereum) {
    try {
      ...
      // Lưu địa chỉ ví vào Supabase để nhận diện khách hàng
      await supabase.from('profiles').insert([{ wallet_address: wallet }]);
      alert("✅ Đã kết nối ví: " + wallet.substring(0, 6) + "...");
      setShowAuthModal(false);
    } catch (err) { alert("Từ chối kết nối ví!"); }
  } else {
...
<nav style={styles.navbar}>
  ...
  <div style={styles.navLinks}>
    <a href="#discover" style={styles.navItem}>Khám phá</a>
    
    {/* NHÓM NÚT ĐĂNG NHẬP */}
    <div style={styles.authGroup}>
      <button style={styles.btnNavText} onClick={() => setShowAuthModal(true)}>
        📧 Email Login
      </button>
      <div style={styles.divider}></div>
      <button style={styles.btnConnect} onClick={() => setShowAuthModal(true)}>
        🦊 Connect Wallet
      </button>
    </div>
  </div>
  ...
  <div style={styles.priceContainer}>
    <span style={styles.priceLabel}>Giá hiện tại</span>
    <div style={styles.priceValue}>
      <img src="https://cryptologos.cc" style={{width: '14px'}} /> 
      {nft.price || '0.01'} MATIC
    </div>
  </div>
...
{showAuthModal && (
  <div style={styles.modalOverlay} onClick={() => setShowAuthModal(false)}>
    <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
      ...
      <h2 style={styles.modalTitle}>Tham gia <br/> <span style={{color: '#6366f1'}}>Hùng Louis Studio</span></h2>
      <p style={styles.modalSubtitle}>Chọn phương thức để kết nối với nghệ sĩ</p>

      {/* LỰA CHỌN 1: EMAIL (TRUYỀN THỐNG) */}
      <div style={styles.authSection}>
        <label style={styles.modalLabel}>📧 Đăng ký bằng Email</label>
        <div style={{display: 'flex', gap: '10px'}}>
          ...
          />
          <button style={styles.btnSmall} onClick={handleRegisterEmail}>Gửi</button>
        </div>
      </div>

      <div style={styles.orDivider}>HOẶC</div>

      {/* LỰA CHỌN 2: VÍ CRYPTO (THỜI THƯỢNG) */}
      <div style={styles.authSection}>
        <label style={styles.modalLabel}>🦊 Kết nối Web3</label>
        <button style={styles.btnWallet} onClick={connectMetamask}>
          <img src="https://upload.wikimedia.org" style={{width: '20px'}} />
          Kết nối Metamask
        </button>
      </div>

      <div style={styles.modalFooter}>Dữ liệu của bạn được bảo mật tuyệt đối.</div>
    </div>
  </div>
)}
{toast.show && (
...
const styles = {
  ...
  navbar: {
    ...
    backgroundColor: 'rgba(5, 5, 5, 0.75)', // Trong suốt hơn
    backdropFilter: 'blur(15px)', // Hiệu ứng kính mờ xịn xò
    zIndex: 5000,
    borderBottom: '1px solid rgba(99, 102, 241, 0.2)', // Viền tím mờ cực sang
    boxShadow: '0 4px 30px rgba(0, 0, 0, 0.5)',
  },
  btnConnect: {
    padding: '10px 25px',
    background: 'linear-gradient(90deg, #6366f1, #a855f7)', // Màu gradient tím xanh
    color: '#fff',
    border: 'none',
    borderRadius: '50px', // Bo tròn hoàn toàn cho duyên dáng
    fontSize: '14px',
    fontWeight: '700',
    cursor: 'pointer',
  ...
  btnNavText: {
   ...
   modalOverlay: {
    ...
    justifyContent: 'center',
    alignItems: 'center',
    zIndex: 9999, // Cho số này thật lớn để luôn nằm trên cùng
  },
  modalContent: {
    backgroundColor: '#111',
    width: '90%',
  ...
  modalTitle: { fontSize: '24px', fontWeight: '800', marginBottom: '10px', lineHeight: '1.4' },
  modalSubtitle: { color: '#888', fontSize: '14px', marginBottom: '30px' },
  inputWrapper: { textAlign: 'left', marginBottom: '25px' },
  modalLabel: { fontSize: '12px', color: '#6366f1', fontWeight: '700', marginBottom: '8px', display: 'block' },
  modalInput: {
    width: '100%',
    padding: '15px',
    backgroundColor: '#1a1a1a',
  ...
  modalFooter: { marginTop: '25px', fontSize: '11px', color: '#555' },
  authSection: { textAlign: 'left', marginBottom: '20px' },
  orDivider: { margin: '20px 0', color: '#444', fontSize: '12px', fontWeight: 'bold' },
  btnSmall: { padding: '10px 20px', backgroundColor: '#6366f1', color: '#fff', border: 'none', borderRadius: '10px', cursor: 'pointer' },
  btnWallet: {
    width: '100%', padding: '15px', backgroundColor: '#fff', color: '#000', 
  ...
  navbar: {
    ...
    justifyContent: 'space-between',
    alignItems: 'center',
    padding: '10px 50px', // Giảm padding để Menu gọn hơn
    position: 'fixed',
    top: '15px', // Cách mép trên một chút cho "sang"
    left: '20px', right: '20px', // Bo hẹp chiều ngang
    borderRadius: '100px', // Bo tròn hoàn toàn
    backgroundColor: 'rgba(15, 15, 15, 0.7)', // Trong suốt hơn
